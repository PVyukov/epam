...
class InfiniteGeneratorWrapper:
    ...
    def __init__(self, g: Generator[str, None, None], max_elements=1000) -> None:
        # insert code here
        self.generator = g
        self.number_of_elements = max_elements
        self.__start = 0

    def __iter__(self): return self

    def __next__(self):
        if self.__start == self.number_of_elements:
            raise StopIteration
        self.__start += 1
        my_var = next(self.generator)
        my_var = my_var[::-1]
        my_var = hexlify(my_var.encode()).decode()
        return my_var
...
class KeepEverythingStartsFrom(BaseRule):
    def __init__(self, starts_from: str):
        super(KeepEverythingStartsFrom, self).__init__()
        self.starts_from = starts_from

    def check(self, item: str) -> bool:
        return item.startswith(self.starts_from)


class DenyEverythingStartsFrom(BaseRule):
    def __init__(self, starts_from: str):
        super(DenyEverythingStartsFrom, self).__init__()
        self.starts_from = starts_from

    def check(self, item: str) -> bool:
        return not item.startswith(self.starts_from)
...
class TestHomework4(unittest.TestCase):
    # The code below doesn't work on Windows, most likely, because of encoding. Use @staticmethod instead.
    # def setUp(self) -> None:
